...
const decrementProductCountStock = `-- name: DecrementProductCountStock :one
UPDATE products
...
type DecrementProductCountStockParams struct {
...
func (q *Queries) DecrementProductCountStock(ctx context.Context, arg DecrementProductCountStockParams) (*Product, error) {
	row := q.db.QueryRow(ctx, decrementProductCountStock, arg.Quantity, arg.ProductID)
	var i Product
	err := row.Scan(
		&i.ProductID,
		&i.MerchantID,
...
const incrementProductCountStock = `-- name: IncrementProductCountStock :one
UPDATE products
SET count_in_stock = count_in_stock + $1,
    updated_at = CURRENT_TIMESTAMP
WHERE product_id = $2
...
type IncrementProductCountStockParams struct {
	Quantity  int32 `json:"quantity"`
	ProductID int32 `json:"product_id"`
}
...
func (q *Queries) IncrementProductCountStock(ctx context.Context, arg IncrementProductCountStockParams) (*Product, error) {
	row := q.db.QueryRow(ctx, incrementProductCountStock, arg.Quantity, arg.ProductID)
	var i Product
	err := row.Scan(
		&i.ProductID,
		&i.MerchantID,
