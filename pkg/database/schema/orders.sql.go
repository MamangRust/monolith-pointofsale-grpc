...
const deleteAllPermanentOrders = `-- name: DeleteAllPermanentOrders :execresult
...
func (q *Queries) DeleteAllPermanentOrders(ctx context.Context) (pgconn.CommandTag, error) {
	return q.db.Exec(ctx, deleteAllPermanentOrders)
}
...
const deleteOrderPermanently = `-- name: DeleteOrderPermanently :one
WITH target AS MATERIALIZED (
    SELECT
        o.order_id,
        o.merchant_id,
...
type DeleteOrderPermanentlyRow struct {
...
func (q *Queries) DeleteOrderPermanently(ctx context.Context, orderID int32) (*DeleteOrderPermanentlyRow, error) {
	row := q.db.QueryRow(ctx, deleteOrderPermanently, orderID)
	var i DeleteOrderPermanentlyRow
	err := row.Scan(
		&i.OrderID,
		&i.MerchantID,
		&i.CashierID,
...
const getAllOrdersTrashed = `-- name: GetAllOrdersTrashed :many
...
func (q *Queries) GetAllOrdersTrashed(ctx context.Context) ([]*Order, error) {
	rows, err := q.db.Query(ctx, getAllOrdersTrashed)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
